...
dta = dta %>% 
  mutate(Hour_St=hour(Start_Time),
...
#First_Cutoff
dtaOrg = dtaOrg %>%
  group_by(OD_Pair,Hour,Minute,Post,PairDist,PU_Tract,DO_Tract,PU_Dist,DO_Dist) %>% 
  summarise()
...
dta15 = left_join(dtaOrg,dta15,by=c('Post',"Hour","Minute","OD_Pair"))
dta15 = dta15 %>% 
  mutate(PairDist=as.numeric(PairDist), 
         ...
         IntTrt=ifelse(Hour%in%c(6,21),1,0))

write.csv(dta15,"/home/keitaro2/Uber_Chicago/Raw_Data/Key_Data/dtaInt15Month.csv")
